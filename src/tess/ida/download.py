...
async def do_ida_range(session, base_url: str, ida_base_dir: str, name: str, since: datetime, until: datetime, N: int) -> None:
    for grp in group(N, month_range(since, until)):
        tasks = [asyncio.create_task(do_ida_single(
            session, base_url, ida_base_dir, name, m, None)) for m in grp]
...
def add_args(parser: ArgumentParser) -> ArgumentParser:
    ...
    parser_single.add_argument('--timeout', type=int, default=300,
    ...
    parser_range.add_argument('-c', '--concurrent', type=int, metavar='<N>', choices=[
                              1, 2, 4, 6, 8], default=4, help='Number of concurrent downloads (defaults to %(default)s)')
    parser_range.add_argument('--timeout', type=int, default=300,
                              help='HTTP timeout in seconds (defaults to %(default)s) sec.')
    ...
    parser_phot.add_argument('-c', '--concurrent', type=int, metavar='<N>', choices=[
                             1, 2, 3, 4], default=4, help='Number of concurrent downloads (defaults to %(default)s)')
    parser_phot.add_argument('--timeout', type=int, default=300,
                             help='HTTP timeout in seconds (defaults to %(default)s) sec.')
    return parser
