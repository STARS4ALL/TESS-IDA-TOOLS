...
def cli_coords_add(args: Namespace) -> None:
    ...
    try:
        with sqlite3.connect(dbase_path) as connection:
            connection.execute(
                'INSERT OR IGNORE INTO coords_t(phot_name, latitude, longitude, height) VALUES(?,?,?,?)', data)
...
def cli_coords_update(args: Namespace) -> None:
    ...
    if not setters:
        ...
        return
    sql = 'UPDATE coords_t SET ' + \
        ', '.join(setters) + ' WHERE phot_name = :name'
    try:
        with sqlite3.connect(dbase_path) as connection:
    ...
    log.warning(
        "[%s] Sun/Moon data no longer valid. Delete your %s ECSV files and re-run the pipeline", args.name, args.name)
...
def cli_coords_delete(args: Namespace) -> None:
    ...
    try:
        with sqlite3.connect(dbase_path) as connection:
            connection.execute(
                'DELETE FROM coords_t WHERE phot_name = ?', data)
    except Exception as e:
        log.error(e)
    connection.close()
...
def add_args(parser: ArgumentParser) -> ArgumentParser:
    ...
    parser_coords = subparser.add_parser(
    ...
    loc_add = subparser.add_parser(
        'add',  help="Add coordinates to a photometer")
    loc_add.add_argument('-n', '--name', type=str,
                         required=True, help='Photometer name')
    loc_add.add_argument('-la', '--latitude', type=float,
                         required=True, help='Latitude [degrees]')
    loc_add.add_argument('-lo', '--longitude', type=float,
                         required=True, help='Longitude [degrees]')
    loc_add.add_argument('-he', '--height', type=float, default=0.0,
                         help='Height above sea level [m] (default: %(default)s)')
    loc_del = subparser.add_parser(
        'delete',  help="Remove coordinates from photometer")
    loc_del.add_argument('-n', '--name', type=str,
                         required=True, help='Photometer name')
    loc_del = subparser.add_parser('list',  help="List photometer coordinates")
    loc_del.add_argument('-n', '--name', type=str,
                         default=None, help='Optional photometer name')
    loc_upd = subparser.add_parser(
        'update',  help="Update coordinates to photometer")
    loc_upd.add_argument('-n', '--name', type=str,
                         required=True, help='Photometer name')
    loc_upd.add_argument('-la', '--latitude', type=float,
                         default=None, help='Latitude [degrees]')
    loc_upd.add_argument('-lo', '--longitude', type=float,
                         default=None, help='Longitude [degrees]')
    loc_upd.add_argument('-he', '--height', type=float, default=None,
                         help='Height above sea level [m] (default: %(default)s)')
    return parser
