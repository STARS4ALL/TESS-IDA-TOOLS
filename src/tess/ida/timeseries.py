...
def ida_metadata(path: str, fix: bool) -> Dict[str, Any]:
    ...
    header[IKW.OBSERVER] = {'observer': v_or_n(
        observer), 'affiliation': v_or_n(affil)}
    ...
    try:
        ...
        if not fix:
            ...
            log.error(
                "[%s] [%s] Add coordinates to aux. coordinates table & re-run with --fix", name, month)
            raise NoCoordinatesError
        coords = next(aux_table_coords_lookup(name))
        if not coords:
            log.error(
                "[%s] [%s] Could not find alternative coordinates in the aux. coordinates table", name, month)
            raise NoCoordinatesError
        ...
        header[IKW.POSITION] = {'latitude': lati,
                                'longitude': longi, 'height': h}
    ...
    if header[IKW.NUM_CHANNELS] == 1:
        ...
        header['Filters per channel'] = [f.strip()[1:-1]
                                         for f in header['Filters per channel'][1:-1].split(',')]
...
def add_columns(table: TimeSeries, name: str, month: str) -> None:
    ...
    table[TS.MOON_ILLUM] = np.round(
        observer.moon_illumination(table['time']), 3)


def create_table(path: str, fix: bool) -> TimeSeries:
    '''Create TimeSeries or loads table from IDA file'''
    name, month = name_month(path)
    log.info("[%s] [%s] Creating a Time Series from IDA file: %s",
             name, month, path)
...
def save_table(table: TimeSeries, path: str) -> None:
    ...
    table.write(path, format='ascii.ecsv', delimiter=',',
...
def do_to_ecsv_single(in_path: str, out_path: str, fix: bool) -> None:
    ...
    if result:
        ...
        if data[1] != stored_hash_str or not os.path.isfile(out_path):
            ...
            log.info("[%s] [%s] Time Series already in ECSV file: %s",
                     name, month, out_path)
    else:
...
def to_ecsv_single(base_dir: OptStr, name: str,  month: OptDate, exact: OptStr, out_dir: str, fix: bool) -> None:
    ...
    filename = name + '_' + \
        month.strftime('%Y-%m') + '.dat' if not exact else exact
...
def to_ecsv_range(base_dir: OptStr,  name: str, out_dir: str, since: datetime, until: datetime, fix: bool) -> None:
    ...
    for path in sorted(glob.iglob(search_path)):
        candidate_month = os.path.splitext(os.path.basename(path))[
            0].split('_')[1]
...
def to_ecsv_combine(base_dir: OptStr,  name: str, since: datetime, until: datetime, oname: str) -> None:
    in_dir_path = to_phot_dir(base_dir, name)
    months = [m for m in month_range(since, until)]
    log.info("[%s] Combining %d months from %s to %s into a single ECSV",
             name, len(months), months[0], months[-1])
    ...
    if len(candidate_path) < 1:
        log.warning(
            "[%s] Not enough tables to combine. Check input parameters.", name)
        return
...
def add_args(parser: ArgumentParser) -> ArgumentParser:
    ...
    parser_comb.add_argument('-n', '--name', type=str,
                             required=True, help='Photometer name')
    parser_comb.add_argument('-i', '--in-dir', type=vdir,
                             default=None, help='Input ECSV base directory')
    parser_comb.add_argument('-s', '--since',  type=vmonth, default=prev_month(
    ), metavar='<YYYY-MM>', help='Year and Month (defaults to %(default)s')
    parser_comb.add_argument('-u', '--until',  type=vmonth, default=cur_month(),
                             metavar='<YYYY-MM>', help='Year and Month (defaults to %(default)s')
    parser_comb.add_argument('-on', '--out-filename', type=str,
                             default=None, help='Optional output combined file name')
    return parser
